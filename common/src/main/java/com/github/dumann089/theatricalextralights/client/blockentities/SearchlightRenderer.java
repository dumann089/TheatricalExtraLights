...
public class SearchlightRenderer extends ExtraLightsFixtureRenderer<SearchlightBlockEntity> {
    private BakedModel cachedPanModel, cachedTiltModel, cachedStaticModel;

    // Local lens exit point for the Mac Vip head.
    // Matches the original renderLensGlow translate used in the old beforeRenderBeam.
    ...
    @Override
    public void renderModel(SearchlightBlockEntity blockEntity, PoseStack poseStack, VertexConsumer vertexConsumer, Direction facing, float partialTicks, boolean isFlipped, BlockState blockState, boolean isHanging, int packedLight, int packedOverlay) {
    ...
    private static final Vec3 LENS_OFFSET = new Vec3(0.5f, 1.28f, -0.265f);
    private static final float MIN_ANGLE_DEG = 1.0f;
    private static final float MAX_ANGLE_DEG = 18.0f;

    @Override
    public void beforeRenderBeam(
            SearchlightBlockEntity blockEntity, PoseStack basePoseStack, VertexConsumer vertexConsumer,
        ...
        if (goboValue == 0) {
            ...
            for (int i = 0; i < beamCount; i++) {
                ...
                submitVolumetricBeam(blockEntity, beamPose, partialTicks, MIN_ANGLE_DEG, MAX_ANGLE_DEG,
                        GoboLibrary.MACVIP, goboValue, focusNorm, 1.0f, 1.0f, i, color, intensityNorm, 0.25f);
            }
        }
        ...
        LazyRenderers.addLazyRender(new LazyRenderers.LazyRenderer() {
            ...
            public void render(MultiBufferSource.BufferSource bufferSource, PoseStack poseStack, Camera camera, float partialTick) {
                ...
                renderLensGlow(builder, poseStack, color, 0.28f);
                poseStack.popPose();

                renderLens(bufferSource, poseStack, alpha, color, 0.28f, (float) LENS_OFFSET.x, (float) LENS_OFFSET.y, (float) LENS_OFFSET.z);
                poseStack.popPose();
            }
            @Override public Vec3 getPos(float partialTick) { return blockEntity.getBlockPos().getCenter(); }
        });
    }

    @Override
    public void preparePoseStack(SearchlightBlockEntity blockEntity, PoseStack poseStack, Direction facing, float partialTicks, boolean isFlipped, BlockState blockState, boolean isHanging) {
