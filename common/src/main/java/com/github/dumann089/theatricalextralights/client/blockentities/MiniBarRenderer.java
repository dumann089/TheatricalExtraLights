...
import com.github.dumann089.theatricalextralights.blockentities.MiniBarBlockEntity;
...
public class MiniBarRenderer extends ExtraLightsRenderer<MiniBarBlockEntity> {
    ...
    @Override
    public void renderModel(MiniBarBlockEntity blockEntity, PoseStack poseStack, VertexConsumer vertexConsumer, Direction facing, float partialTicks, boolean isFlipped, BlockState blockState, boolean isHanging, int packedLight, int packedOverlay) {
    ...
    @Override
    public void beforeRenderBeam(MiniBarBlockEntity blockEntity, PoseStack poseStack, VertexConsumer vertexConsumer, MultiBufferSource multiBufferSource, Direction facing, float partialTicks, boolean isFlipped, BlockState blockstate, boolean isHanging, int packedLight, int packedOverlay) {
        if(blockEntity.getIntensity() > 0){
            LazyRenderers.addLazyRender(new LazyRenderers.LazyRenderer() {
                ...
                public void render(MultiBufferSource.BufferSource bufferSource, PoseStack poseStack, Camera camera, float partialTick) {
                    ...
                    poseStack.translate(0.5, 0.2f, 0.381f);

                    Matrix4f m = poseStack.last().pose();
                    Matrix3f normal = poseStack.last().normal();

                    addVertex(builder, m, normal, r, g, b, a, -0.4375f, 0.0625f, 0f);
                    addVertex(builder, m, normal, r, g, b, a, 0.4375f, 0.0625f, 0f);
                    addVertex(builder, m, normal, r, g, b, a, 0.4375f, -0.0625f, 0f);
                    addVertex(builder, m, normal, r, g, b, a, -0.4375f, -0.0625f, 0f);
                    ...
                    renderLightBeam(builder, poseStack, blockEntity, partialTicks, alpha, 0.4375f, 0.0625f, beamLength, color);
                    poseStack.popPose();
                }
    ...
    protected void renderLightBeam(VertexConsumer builder, PoseStack stack, MiniBarBlockEntity tileEntityFixture, float partialTicks, float alpha, float beamWidth, float beamHeight, float length, int color) {
    ...
    @Override
    public void preparePoseStack(MiniBarBlockEntity blockEntity, PoseStack poseStack, Direction facing, float partialTicks, boolean isFlipped, BlockState blockState, boolean isHanging) {
