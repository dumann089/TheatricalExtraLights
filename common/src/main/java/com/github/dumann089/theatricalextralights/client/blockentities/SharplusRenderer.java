...
public class SharplusRenderer extends ExtraLightsFixtureRenderer<SharplusBlockEntity> {
    ...
    public SharplusRenderer(BlockEntityRendererProvider.Context context) {
        super(context);
    }

    @Override
    public void renderModel(SharplusBlockEntity blockEntity, PoseStack poseStack, VertexConsumer vertexConsumer, Direction facing, float partialTicks, boolean isFlipped, BlockState blockState, boolean isHanging, int packedLight, int packedOverlay) {
    ...
    private static final Vec3 LENS_OFFSET = new Vec3(0.5f, 0.804f, 0.148f);
    private static final float MIN_ANGLE_DEG = 1.0f;
    private static final float MAX_ANGLE_DEG = 15.0f;

    @Override
    public void beforeRenderBeam(
            SharplusBlockEntity blockEntity, PoseStack basePoseStack, VertexConsumer vertexConsumer,
        ...
        float spreadAngleDeg = 1.0f + (45.0f - 1.0f) * zoomT; // Mínimo 1 grado, máximo 45 grados de apertura
        ...
        LazyRenderers.addLazyRender(new LazyRenderers.LazyRenderer() {
            ...
            public void render(MultiBufferSource.BufferSource bufferSource, PoseStack poseStack, Camera camera, float partialTick) {
                ...
                renderLens(bufferSource, poseStack, alpha, color, 0.2f, (float) LENS_OFFSET.x, (float) LENS_OFFSET.y, (float) LENS_OFFSET.z);
                poseStack.popPose();
            }
    ...
    @Override
    public void preparePoseStack(SharplusBlockEntity blockEntity, PoseStack poseStack, Direction facing, float partialTicks, boolean isFlipped, BlockState blockState, boolean isHanging) {
