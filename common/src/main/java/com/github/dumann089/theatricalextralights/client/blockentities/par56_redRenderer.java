...
import com.github.dumann089.theatricalextralights.blockentities.par56_redBlockEntity;
...
public class par56_redRenderer extends ExtraLightsFixtureRenderer<par56_redBlockEntity> {
    private BakedModel cachedPanModel, cachedTiltModel, cachedStaticModel;

    private final WeakHashMap<par56_redBlockEntity, GoboGPUProjector> goboProjectors = new WeakHashMap<>();
    private final WeakHashMap<par56_redBlockEntity, VolumetricBeamRenderer> volumetricRenderers = new WeakHashMap<>();
    private final Double beamOpacity = TheatricalConfig.INSTANCE.CLIENT.beamOpacity;

    public par56_redRenderer(BlockEntityRendererProvider.Context context) {
        super(context);
    }

    @Override
    public void renderModel(par56_redBlockEntity blockEntity, PoseStack poseStack, VertexConsumer vertexConsumer, Direction facing, float partialTicks, boolean isFlipped, BlockState blockState, boolean isHanging, int packedLight, int packedOverlay) {
    ...
    @Override
    public void beforeRenderBeam(par56_redBlockEntity blockEntity, PoseStack poseStack, VertexConsumer vertexConsumer, MultiBufferSource multiBufferSource, Direction facing, float partialTicks, boolean isFlipped, BlockState blockstate, boolean isHanging, int packedLight, int packedOverlay) {
    ...
    private void renderFakeVolumetricBeams(VertexConsumer builder, PoseStack poseStack, par56_redBlockEntity blockEntity, Camera camera, float partialTick, float alpha, int color, int goboSlot) {
