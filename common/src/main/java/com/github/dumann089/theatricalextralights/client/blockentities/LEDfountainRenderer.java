...
import com.github.dumann089.theatricalextralights.blockentities.LEDFountainBlockEntity;
...
public class LEDfountainRenderer extends ExtraLightsFixtureRenderer<LEDFountainBlockEntity> {
    private BakedModel cachedPanModel, cachedTiltModel, cachedStaticModel;

    private final WeakHashMap<LEDFountainBlockEntity, GoboGPUProjector> goboProjectors = new WeakHashMap<>();
    private final WeakHashMap<LEDFountainBlockEntity, VolumetricBeamRenderer> volumetricRenderers = new WeakHashMap<>();
    private final Double beamOpacity = TheatricalConfig.INSTANCE.CLIENT.beamOpacity;

    public LEDfountainRenderer(BlockEntityRendererProvider.Context context) {
        super(context);
    }

    @Override
    public void renderModel(LEDFountainBlockEntity blockEntity, PoseStack poseStack, VertexConsumer vertexConsumer, Direction facing, float partialTicks, boolean isFlipped, BlockState blockState, boolean isHanging, int packedLight, int packedOverlay) {
    ...
    @Override
    public void beforeRenderBeam(LEDFountainBlockEntity blockEntity, PoseStack poseStack, VertexConsumer vertexConsumer, MultiBufferSource multiBufferSource, Direction facing, float partialTicks, boolean isFlipped, BlockState blockstate, boolean isHanging, int packedLight, int packedOverlay) {
        if(blockEntity.getIntensity() <= 0) return;

        Vec3 localLensOffset = new Vec3(0.5f, 1.75f, 0.28f);
        ...
        if (TheatricalExtraLightsConfig.isVolumetricBeamEnabled()) {
            ...
            float coneHalfAngle = 1.0f + zoomNorm * (45.0f - 1.0f);
        ...
        LazyRenderers.addLazyRender(new LazyRenderers.LazyRenderer() {
            ...
            public void render(MultiBufferSource.BufferSource bufferSource, PoseStack poseStack, Camera camera, float partialTick) {
                ...
                poseStack.pushPose();
                poseStack.translate(localLensOffset.x, localLensOffset.y, localLensOffset.z);
                renderLensGlow(builder, poseStack, color, 0.14f);
                poseStack.popPose();

                renderLens(bufferSource, poseStack, alpha, color, 0.14f, (float)localLensOffset.x, (float)localLensOffset.y, (float)localLensOffset.z);
                poseStack.popPose();
            }
    ...
    private void renderFakeVolumetricBeams(VertexConsumer builder, PoseStack poseStack, LEDFountainBlockEntity blockEntity, Camera camera, float partialTick, float alpha, int color, int goboSlot) {
    ...
    @Override
    public void preparePoseStack(LEDFountainBlockEntity blockEntity, PoseStack poseStack, Direction facing, float partialTicks, boolean isFlipped, BlockState blockState, boolean isHanging) {
        //#region Fixture Hanging
