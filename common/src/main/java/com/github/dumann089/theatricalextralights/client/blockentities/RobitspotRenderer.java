...
public class RobitspotRenderer extends ExtraLightsFixtureRenderer<RobitspotBlockEntity> {
    ...
    public RobitspotRenderer(BlockEntityRendererProvider.Context context) {
        super(context);
    }

    @Override
    public void renderModel(RobitspotBlockEntity blockEntity, PoseStack poseStack, VertexConsumer vertexConsumer, Direction facing, float partialTicks, boolean isFlipped, BlockState blockState, boolean isHanging, int packedLight, int packedOverlay) {
    ...
    private static final Vec3 LENS_OFFSET = new Vec3(0.5f, 1.455f, -0.59f);
    private static final float MIN_ANGLE_DEG = 1.0f;
    private static final float MAX_ANGLE_DEG = 15.0f;

    @Override
    public void beforeRenderBeam(
            RobitspotBlockEntity blockEntity, PoseStack basePoseStack, VertexConsumer vertexConsumer,
        ...
        LazyRenderers.addLazyRender(new LazyRenderers.LazyRenderer() {
            ...
            public void render(MultiBufferSource.BufferSource bufferSource, PoseStack poseStack, Camera camera, float partialTick) {
                ...
                renderLensGlow(builder, poseStack, color, 0.19f);
                poseStack.popPose();

                renderLens(bufferSource, poseStack, alpha, color, 0.25f, (float) LENS_OFFSET.x, (float) LENS_OFFSET.y, (float) LENS_OFFSET.z);
                poseStack.popPose();
            }
    ...
    @Override
    public void preparePoseStack(RobitspotBlockEntity blockEntity, PoseStack poseStack, Direction facing, float partialTicks, boolean isFlipped, BlockState blockState, boolean isHanging) {
