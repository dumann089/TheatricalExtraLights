...
public class SpotXtremeGoboRenderer extends ExtraLightsFixtureRenderer<SpotXtremeGoboBlockEntity> {
    ...
    public SpotXtremeGoboRenderer(BlockEntityRendererProvider.Context context) {
        super(context);
    }

    private float[] getThrottledStructuralTransforms(SpotXtremeGoboBlockEntity be, BlockState state) {
    ...
    @Override
    public void renderModel(SpotXtremeGoboBlockEntity blockEntity, PoseStack poseStack, VertexConsumer vertexConsumer, Direction facing, float partialTicks, boolean isFlipped, BlockState blockState, boolean isHanging, int packedLight, int packedOverlay) {
    ...
    public void beforeRenderBeam(
            SpotXtremeGoboBlockEntity blockEntity,
            PoseStack poseStack,
        ...
        if (blockEntity.getGobo() >= 0) {
            ...
            if (TheatricalExtraLightsConfig.isVolumetricBeamEnabled()) {
                ...
                ResourceLocation goboTex = GoboLibrary.SpotXtreme.getTexture(blockEntity.getGobo());
