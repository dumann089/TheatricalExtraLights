...
import com.github.dumann089.theatricalextralights.blockentities.par56_yellowBlockEntity;
...
public class par56_yellowRenderer extends ExtraLightsFixtureRenderer<par56_yellowBlockEntity> {
    private BakedModel cachedPanModel, cachedTiltModel, cachedStaticModel;

    private final WeakHashMap<par56_yellowBlockEntity, GoboGPUProjector> goboProjectors = new WeakHashMap<>();
    private final WeakHashMap<par56_yellowBlockEntity, VolumetricBeamRenderer> volumetricRenderers = new WeakHashMap<>();
    private final Double beamOpacity = TheatricalConfig.INSTANCE.CLIENT.beamOpacity;

    public par56_yellowRenderer(BlockEntityRendererProvider.Context context) {
        super(context);
    }

    @Override
    public void renderModel(par56_yellowBlockEntity blockEntity, PoseStack poseStack, VertexConsumer vertexConsumer, Direction facing, float partialTicks, boolean isFlipped, BlockState blockState, boolean isHanging, int packedLight, int packedOverlay) {
    ...
    @Override
    public void beforeRenderBeam(par56_yellowBlockEntity blockEntity, PoseStack poseStack, VertexConsumer vertexConsumer, MultiBufferSource multiBufferSource, Direction facing, float partialTicks, boolean isFlipped, BlockState blockstate, boolean isHanging, int packedLight, int packedOverlay) {
    ...
    private void renderFakeVolumetricBeams(VertexConsumer builder, PoseStack poseStack, par56_yellowBlockEntity blockEntity, Camera camera, float partialTick, float alpha, int color, int goboSlot) {
    ...
    @Override
    public void preparePoseStack(par56_yellowBlockEntity blockEntity, PoseStack poseStack, Direction facing, float partialTicks, boolean isFlipped, BlockState blockState, boolean isHanging) {
        //#region Fixture Hanging
