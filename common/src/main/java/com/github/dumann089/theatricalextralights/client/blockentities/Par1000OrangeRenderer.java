...
import com.github.dumann089.theatricalextralights.blockentities.Par1000OrangeBlockEntity;
...
public class Par1000OrangeRenderer extends ExtraLightsFixtureRenderer<Par1000OrangeBlockEntity> {
    ...
    public Par1000OrangeRenderer(BlockEntityRendererProvider.Context context) {
        super(context);
    }

    @Override
    public void renderModel(Par1000OrangeBlockEntity blockEntity, PoseStack poseStack, VertexConsumer vertexConsumer, Direction facing, float partialTicks, boolean isFlipped, BlockState blockState, boolean isHanging, int packedLight, int packedOverlay) {
    ...
    private static final float MIN_ANGLE_DEG = 9.0f;
    private static final float MAX_ANGLE_DEG = 9.0f;

    @Override
    public void beforeRenderBeam(
            Par1000OrangeBlockEntity blockEntity,
            PoseStack poseStack,
    ...
    @Override
    public void preparePoseStack(Par1000OrangeBlockEntity blockEntity, PoseStack poseStack, Direction facing, float partialTicks, boolean isFlipped, BlockState blockState, boolean isHanging) {
        poseStack.translate(0.5F, 0, .5F);
        if (isHanging) {
            Direction hangDirection = blockState.getValue(HangableBlock.HANG_DIRECTION);
            poseStack.translate(0, 0.5, 0F);
