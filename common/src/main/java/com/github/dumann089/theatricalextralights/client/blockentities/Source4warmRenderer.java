...
public class Source4warmRenderer extends ExtraLightsFixtureRenderer<Source4warmBlockEntity> {
    private BakedModel cachedPanModel, cachedTiltModel, cachedStaticModel;

    // Local lens exit point — matches the original beforeRenderBeam translate.
    private static final Vec3 LENS_OFFSET = new Vec3(0.5f, 0.5f, 0.122f);
    ...
    public Source4warmRenderer(BlockEntityRendererProvider.Context context) {
        super(context);
    }

    @Override
    public void renderModel(Source4warmBlockEntity blockEntity, PoseStack poseStack, VertexConsumer vertexConsumer, Direction facing, float partialTicks, boolean isFlipped, BlockState blockState, boolean isHanging, int packedLight, int packedOverlay) {
    ...
    private static final float MIN_ANGLE_DEG = 1.5f;
    private static final float MAX_ANGLE_DEG = 1.5f;

    @Override
    public void beforeRenderBeam(
            Source4warmBlockEntity blockEntity,
            PoseStack poseStack,
        ...
        LazyRenderers.addLazyRender(new LazyRenderers.LazyRenderer() {
            ...
            public void render(MultiBufferSource.BufferSource bufferSource, PoseStack poseStack, Camera camera, float partialTick) {
                ...
                renderLens(bufferSource, poseStack, alpha, color,
                        0.58f, (float) LENS_OFFSET.x, (float) LENS_OFFSET.y, (float) LENS_OFFSET.z);
