...
import com.github.dumann089.theatricalextralights.blockentities.par56_warmBlockEntity;
...
public class par56_warmRenderer extends ExtraLightsFixtureRenderer<par56_warmBlockEntity> {
    private BakedModel cachedPanModel, cachedTiltModel, cachedStaticModel;

    private final WeakHashMap<par56_warmBlockEntity, GoboGPUProjector> goboProjectors = new WeakHashMap<>();
    private final WeakHashMap<par56_warmBlockEntity, VolumetricBeamRenderer> volumetricRenderers = new WeakHashMap<>();
    private final Double beamOpacity = TheatricalConfig.INSTANCE.CLIENT.beamOpacity;

    public par56_warmRenderer(BlockEntityRendererProvider.Context context) {
        super(context);
    }

    @Override
    public void renderModel(par56_warmBlockEntity blockEntity, PoseStack poseStack, VertexConsumer vertexConsumer, Direction facing, float partialTicks, boolean isFlipped, BlockState blockState, boolean isHanging, int packedLight, int packedOverlay) {
    ...
    @Override
    public void beforeRenderBeam(par56_warmBlockEntity blockEntity, PoseStack poseStack, VertexConsumer vertexConsumer, MultiBufferSource multiBufferSource, Direction facing, float partialTicks, boolean isFlipped, BlockState blockstate, boolean isHanging, int packedLight, int packedOverlay) {
    ...
    private void renderFakeVolumetricBeams(VertexConsumer builder, PoseStack poseStack, par56_warmBlockEntity blockEntity, Camera camera, float partialTick, float alpha, int color, int goboSlot) {
    ...
    @Override
    public void preparePoseStack(par56_warmBlockEntity blockEntity, PoseStack poseStack, Direction facing, float partialTicks, boolean isFlipped, BlockState blockState, boolean isHanging) {
        //#region Fixture Hanging
